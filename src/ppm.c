				...
				for (int y1 = start; y1 < end; ++y1)
				{
					...
					for (int x1 = 0; x1 < width; ++x1)
					{
						...
						for (size_t i = 0; i < print_rate; ++i)
						{
							...
							path_trace(x1, y1, local_y, width, &scene, bounces, local_color_buffer, &seed_per_threads, 0);
						}
...
void compute_russian(pt_config_t *config)
{
	unsigned int seed = (unsigned int)time(NULL);
				...
				for (int y1 = start; y1 < end; ++y1)
				{
					...
					for (int x1 = 0; x1 < width; ++x1)
					{
						...
						for (size_t i = 0; i < print_rate; ++i)
						{
							...
							path_trace(x1, y1, local_y, width, &scene, bounces, local_color_buffer, &seed_per_threads, 1);
						}
...
/* ============================================================================
 *  compute_clusters_no_rr
 * ========================================================================== */

void compute_clusters_no_rr(pt_config_t *config)
{
	unsigned int seed = (unsigned int)time(NULL);
...
#pragma omp for schedule(static)
				for (int y1 = start; y1 < end; ++y1)
				{
					...
					for (int x1 = 0; x1 < width; ++x1)
					{
						...
						{
							path_trace_clusters(x1, y1, local_y, width, &scene, bounces,
												&pixel_color, &seed_per_threads, tree, 0);
						}
...
void compute_bdpt_no_rr(pt_config_t *config)
{
	unsigned int seed = (unsigned int)time(NULL);
